...
local = Path("C:/harfbuzz-local")
conda = [sys.executable, "C:/harfbuzz-tools/Scripts/conda-script.py"]
...
run(conda + ["build", "recipe", "-m", f".ci_support/{subdir.replace('-', '_')}_.yaml",
             ...
             "--croot", "C:/harfbuzz-build", "--output-folder", str(local), "--no-anaconda-upload",
             "--override-channels", "-c", "file:///C:/harfbuzz-local", "-c", "conda-forge"])
print(f"PASS: harfbuzz build and installed consumer tests on {subdir}", flush=True)
